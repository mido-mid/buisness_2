...
<!-- given emotions like, wow, sad (default:Like) -->
        ...
        <span class="like-details" id="like-details-{{$model->id}}" data-toggle="modal" data-target="#likes-modal-{{$model->id}}">@if($model->likes->count-1 > 0) {{__('home.and')}} {{$model->likes->count-1}} @if($model->likes->count-1 > 1000) {{__('home.thousand')}} @endif {{__('home.others')}} @endif</span>
    </div>
