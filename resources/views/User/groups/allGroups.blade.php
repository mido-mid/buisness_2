                            ...
                            <div class="card">
                                ...
                                <div class="d-flex justify-content-between">
                                    <div class="card-body">
                                        ...
                                        <p class="card-text"><small class="text-muted" id="{{$all_group->id}}">
                                            <?php
                                                $member = App\models\GroupMember::where('group_id',$all_group->id)->count();
                                                echo $member;
                                            ?>
                                    ...
                                    <div class="p-2">
                                        @if(Auth::guard('web')->user())
                                            ...
                                            <div class="p-2">
                                                    <button class="button-4 totyAllgroups" id="join|{{$all_group->id}}" >{{__('groups.join')}} </button>
                                            </div>

                                            @elseif (count($checkState)>0)
                                                @if ($checkState[0]->state == 1)
                                                    <div class="p-2">
                                                            <button class="button-2 totyAllgroups" id="leave|{{$all_group->id}}">{{__('groups.left')}}</button>
                                                    </div>

                                                @elseif ($checkState[0]->state == 2)
                                                    <div class="p-2">
                                                        <button class="button-2 totyAllgroups" id="leave|{{$all_group->id}}">{{__('groups.left_request')}}</button>
                                                    </div>
